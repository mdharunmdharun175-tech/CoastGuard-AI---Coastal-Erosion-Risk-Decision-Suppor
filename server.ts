...
function computeErosionAndShap(features: {
  ...
  // Normalized deviations (z-scores relative to regional baselines)
  ...
  const z_doc = (BASELINES.depth_of_closure - depth_of_closure) / 3.8; // shallower closure depth increases erosion
  ...
  // Exact TreeSHAP marginal contributions (phi_i)
  ...
  // Synergistic non-linear hydrodynamic interaction (wave energy flux * steep slope)
...
// -------------------------------------------------------------
// Real sample coastal transects (Adriatic Costa dei Trabocchi, Tyrrhenian, Ligurian, etc.)
// -------------------------------------------------------------
...
// GET /api/zones - returns all monitored coastal zones with computed risk & SHAP
...
// GET /api/benchmark - comparative academic metrics vs Azzara et al. (2026) MARS
...
// GET /api/dataset.csv - export research dataset as CSV file download
...
// GET /api/alerts/3-day - 3-day early warning disaster forecast evaluator
...
// GET /api/history - Retrieve previous historical surveys and extreme storm event logs
...
// GET /api/weather/live - Real-time Weather Telemetry & 72-Hour Hydrodynamic Predictions
app.get("/api/weather/live", (req: Request, res: Response) => {
  ...
  // Base wave height calibrated by transect storm energy
...
// GET /api/satellite/telemetry - Retrieve Recent Satellite Telemetry Indicators (NDVI, NDWI, Shoreline Shifts, SAR Coherence)
app.get("/api/satellite/telemetry", (req: Request, res: Response) => {
  ...
  // Calibrate vegetation NDVI: Low on eroding dunes / cliffs, high on vegetated reserves
  ...
  // Optical Water Indices (NDWI & MNDWI)
  ...
  // Shoreline displacement & morphological indicators
  ...
  // Radar SAR Coherence & Backscatter
  ...
  // Recent Multi-Date Overpasses Time-Series (Last 6 Sentinel / Landsat passes)
  const now = Date.now();
  const recent_overpasses = [
    ...
    },
    {
      ...
      cloud_pct: 0.0, // Radar all-weather
      quality_score: 98,
  ...
  res.json({
    ...
    // Shoreline Extraction & Morphodynamics
...
// Helper: Offline Intelligent Oceanographic Assessment Generator (for graceful resilience when API Key is pending)
function generateFallbackAssessment(transect: any, climate_scenario?: string, user_prompt?: string): string {
...
// Helper: Offline Intelligent Chat Copilot Response Generator
...
// POST /api/ai/advisor - Comprehensive Gemini Coastal Vulnerability Assessment & Mitigation Strategy
...
${user_prompt ? `User Specific Focus: ${user_prompt}` : "Provide optimal multi-tier engineering & NBS strategy."}`;
  ...
  // Graceful intelligent oceanographic fallback when API key is unconfigured or rate limited
...
// POST /api/ai/chat - Interactive Coastal Copilot Chat
app.post("/api/ai/chat", async (req: Request, res: Response) => {
  ...
  // Graceful intelligent copilot fallback when API key is unconfigured or rate limited
